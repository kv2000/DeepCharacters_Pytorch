...
class WootCharacter(nn.Module):
    def __init__(
        ...
        # the laplacian loss related stuff
        ...
        # the edge ios loss realted stuff
    ...
    def build_embedded_graph(self, graph_dir): 
        ...
        #####################################################################
        # so time to add, the node to node linke is for the arap loss
        ...
        #####################################################################
        # then set up the links between node 
        ...
        for i in range(self.graph_verts_num):
            ...
            # add the neighbors on the templatemesh to the graph
    ...
    def compute_embedded_graph_deformation(self, blended_mat = None, deltaR = None, deltaT = None, perVertex_displacement=None):
        ...
        batch_size = blended_mat.shape[0]
        # then we can get the setting
        org_template = self.temp_verts
        #org_graph = self.graph_verts  # <--- should never be used ...
        
        # so we always do the non-rigid deofmration for egnet
        deltaR_mat = batch_angle_to_mat(deltaR)

        # pick up the template vertiecs
        picked_temp_verts = org_template[self.link_temp_id,:]
        # this should be the vertex that the node linked to 
        ...
        # then we compute the template related stuff
